...
//$path = 'C:\Users\ThanhNV\Desktop\TMP\images_test';
$path = '';
...
$txt_color = (isset($_POST['txt_color'])) ? $_POST['txt_color'] : '000000';
if(isset($_POST['path'])) {
		...
		if(count($files)) {
			...
			foreach ($files as $file) {
				...
				switch ($ext) {
					...
					case 'jpg': 
					case 'jpeg': 
						imagejpeg($im, $file); break;
				}
				imagedestroy($im);
				
				$msg .= "Successfully relace image {$file}<br />";
			}
		}
	}
...
function jaFiles($path, $filter = '.', $recurse = false, $fullpath = false, $exclude = array('.svn', 'CVS'))
{
	$exclude[] = '.svn';
	$exclude[] = '.git';
	$exclude[] = 'CVS';
	...
	$arr = _jaFiles($path, $path, $filter, $recurse, $fullpath, $exclude);
	if(!$arr) {
		$arr = array();
	}
	return $arr;
...
function _jaFiles($path, $basePath, $filter = '.', $recurse = false, $fullpath = false, $exclude = array('.svn', 'CVS'))
{
			...
			if ($isDir) {
				if ($recurse) {
					if (is_integer($recurse)) {
						$arr2 = _jaFiles($dir, $basePath, $filter, $recurse - 1, $fullpath, $exclude);
					} else {
						$arr2 = _jaFiles($dir, $basePath, $filter, $recurse, $fullpath, $exclude);
					}
...
<?php
$aBgFiles = jaFiles(BG_PATH, "\.(jpg|jpeg|gif|png|bmp|wbmp)$");
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>JoomlArt Hide Image Tool</title>
<style type="text/css">
#result {padding:10px 20px; border:1px dashed #CC6600; line-height:18px;}
.group {font-weight:bold; color:red; padding: 3px 5px;}
.title {color: blue;}
</style>
...
<option value="<?php echo $bg; ?>" <?php if($bg == $bg_image) {echo 'selected="selected"';} ?>><?php echo $bg; ?></option>
<?php endforeach; ?>
<?php endif; ?>
</select>
